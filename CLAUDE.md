...
## ⚖️ 法律合規最高準則(2026-07-09 訂,不可違反)

**《食品安全衛生管理法》是本站一切標示功能的最高準則**。全文副本與條文對照、
風險模型、免責標準文字都在 **`docs/legal/compliance.md`**(法規全文:
`docs/legal/食品安全衛生管理法.md`)。開發鐵律:

1. 任何「輸出可能被印上包裝」的功能(標示下載、標籤、列印)**必須**同時具備:
   斜向浮水印 + 圖內烙印免責聲明 + 下載前一次性同意(localStorage 留同意時間)。
2. 免責文字只能加強、不得刪弱;修改需同步 `compliance.md` 第四節、
   `labelImage.js`、`Detail.jsx` 三處。
3. 關鍵數字:標示不實罰鍰 4 萬~400 萬(第 45 條);誤差容許:限制性營養素
   實測 ≤ 標示值 120%、蛋白質 ≥ 80%。烘焙失水濃縮是最大技術風險,
   所以成品重(`finishedGrams`)機制不可移除。
...
**完整路線圖見 `docs/roadmap.md`(2026-07-09 與使用者盤點,尚未實作,等需求逐項確定)**:
...
- **公開的材料營養/過敏原資料庫是未來主力功能**:廠牌+規格+標示日期的市售品項資料,
  是衛福部通用資料查不到的價值。
- 較小的待辦:標籤輸出(內容物依重量遞減+過敏原+有效日期)、半成品(食譜當材料引用)、
  進貨紀錄/價格歷史(需帳號)。
...
涉及網站功能/介面的 push:`package.json` 的 `version` 依語意化版本 +1(主=架構變動、
次=新功能、修訂=修 bug/微調),並在 `CHANGELOG.md` 加一段紀錄(新的在上)。前端會自動從
package.json 讀版本顯示在側欄底部,不用改程式。純文件(docs/README)變更不 bump。
